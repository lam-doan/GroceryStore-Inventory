...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  while( fin >> std::quoted( upc ) >> std::ws >> quantity )
  {
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  double amount = 0;
  for ( const auto&[upc, item] : shoppingCart ) {
    // item not found
    auto grocery_item = worldWideGroceryDatabase.find( upc );
    if( grocery_item == nullptr )
    {
      receipt << ' ' << std::quoted( upc ) << " (" << item.productName() << ") " << "not found, the item is free!\n";
    }
    else
    {
      receipt << ' ' << *grocery_item << '\n';
      // add to the amount due
      amount += grocery_item->price();
      if( auto it = _inventoryDB.find( upc ); it != _inventoryDB.end() )
      {
        // update the number of items in inventory
        -- it->second;
        // add item's upc to purchasedGroceries
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  {
    auto item  = _inventoryDB.find( upc );
    if( item == _inventoryDB.end() || ( found && item->second < REORDER_THRESHOLD ) )
    {
      reorderReport << ++count << ": ";
      auto grocery_item = worldWideGroceryDatabase.find( upc );
      if( grocery_item == nullptr ) reorderReport << upc << '\n';
      else
      {
        reorderReport << "{" << *grocery_item << "}\n";
      }

      if( item = _inventoryDB.end() ) { reorderReport << "\n       *** no longer sold in this store and will not be re-ordered\n"; }
      else {
        reorderReport << "       only "
                      << item->second
                      << " remain in stock which is "
                      << ( REORDER_THRESHOLD - item->second )
                      << " unit(s) below reorder threshold ("
                      << REORDER_THRESHOLD << "), reordering "
                      << LOT_COUNT << " more" << "\n\n";
        _inventoryDB[upc] += LOT_COUNT;
      }
    }
  }
  todaysSales.clear();
